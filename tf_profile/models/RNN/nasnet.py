...
def cifar_config():
    return tf.contrib.training.HParams(
        ...
        # This is used for the drop path probabilities since
...
def nasnet_cifar_arg_scope(weight_decay=5e-4,
    ...
    Args:
      ...
      batch_norm_epsilon: Small float added to variance
...
def nasnet_large_arg_scope(weight_decay=5e-5,
    ...
    Args:
      weight_decay: The weight decay to use for regularizing the model.
      batch_norm_decay: Decay for batch norm moving average.
      batch_norm_epsilon: Small float added to variance to
...
def NasNetFn(batchsize=None):
    ...
    with slim.arg_scope(nasnet_mobile_arg_scope()):
        logits, end_points = build_nasnet_mobile(inputs, num_classes)
...
def NasNetFn_horovod(batchsize=None):
    batch_size = 5
    # use specified batch_size if not None
    if batchsize is not None:
        batch_size = batchsize
